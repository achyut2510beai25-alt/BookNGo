// ============================================================
//  BookNGo — script.js
//  Syllabus coverage: Variables, Data Types, Operators,
//  Control Flow, Functions, Arrow Functions, Closures,
//  Arrays, Objects, HOF (map/filter/reduce/sort),
//  Destructuring, JSON, DOM, Events, Form Validation,
//  BOM, localStorage, Promises (simulated async)
// ============================================================
...
const eventsData = [
  ...
  { id:11, name: "Mughal-E-Azam Musical",  category: "theatre", city: "Mumbai",    date: "2026-06-01", price: 500  },
  { id:12, name: "Zakir Khan Live",        category: "theatre", city: "Delhi",     date: "2026-05-18", price: 800  },
];
...
function confirmBooking() {
  ...
  // Destructuring the first selected seat
  const [firstSeat] = selectedSeats;

  showAlert("⏳ Processing your booking…", "info");

  // Promise with setTimeout to simulate network call
  ...
  bookingPromise
    .then(data => {
      // Destructuring from resolved object
...
function showBookingConfirmation(name, seats, ref) {
  ...
  document.getElementById("conf-seats").textContent =
    seats.map(id => {
      const [sec, r, c] = id.split("-");
      return `${sec} ${String.fromCharCode(65+Number(r))}${Number(c)+1}`;
    }).join(", ");

  overlay.style.display = "flex";

  // Event Delegation — close on overlay click
...
// ── SECTION 14: Filters on Category Pages ────────────────────
...
function initFilters() {
  ...
  // Populate cities using map
  const pageCategory = document.body.dataset.category || "all";

  // Fill city dropdown using for...of
  const cities = [...new Set(
    eventsData
      .filter(e => pageCategory === "all" || e.category === pageCategory)
      .map(e => e.city)
  )];
  ...
  function renderFiltered() {
    ...
    const data   = filterEvents(pageCategory, city, sortBy);
    ...
    // map to create card HTML strings, join & set innerHTML
    container.innerHTML = data
      .map(e => `
        <div class="big-card">
          ...
          <p>📍 ${e.city}</p>
          <div class="tickets">
            <div class="ticket-type">General<span>${formatCurrency(e.price)}</span></div>
            <div class="ticket-type">VIP<span>${formatCurrency(e.price * 3)}</span></div>
          </div>
          <p class="interested" data-countdown="${e.date}">⏳ Loading timer…</p>
          <a href="booking.html" class="btn">Book Now</a>
        </div>`)
      .join("");

    startCountdownTimers(); // restart timers on new cards
  }
}

// ── SECTION 15: BOM — Window & Navigator ─────────────────────
...
function showBrowserInfo() {
  ...
  console.table(info); // console.table from syllabus
}

// console methods demo (syllabus requirement)
console.log(`${APP_NAME} v${VERSION} loaded`);
console.warn("Demo mode — no real payments processed");
console.table(eventsData.slice(0, 3));

// ── SECTION 16: DOMContentLoaded — Init ───────────────────────
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Event Delegation on document for confirm-booking button
  document.addEventListener("click", e => {
    if (e.target.id === "confirm-btn") confirmBooking();
    if (e.target.id === "logout-btn") logout();
    if (e.target.id === "send-msg-btn") validateContact();
    if (e.target.id === "login-btn-id") login();
    if (e.target.id === "signup-btn-id") signup();
    if (e.target.id === "close-confirm") closeConfirm();
  });

  // for...in loop over eventsData object keys (demo)
  for (const key in eventsData[0]) {
    // just iterating to show for...in usage
  }
});

// Window onload — BOM
window.addEventListener("load", () => {
  console.log("Window fully loaded — all assets ready");
});